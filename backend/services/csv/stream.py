...
# Import normalized helpers from sibling modules
...
def stream_ticks(
    ...
    start_naive = start_t.tz_convert("UTC").tz_localize(None) if start_t.tzinfo else start_t
    end_naive   = end_t.tz_convert("UTC").tz_localize(None)   if end_t.tzinfo   else end_t

    approx_bytes_per_row = max(30, (get_file_size(csv_path) - data_start) // max(1, _estimate_total_rows(csv_path, data_start)))
                    ...
                    try:
                        ...
                        if not np.issubdtype(times.dtype, np.datetime64):
                            try:
                                ...
                                times_dt = np.array([np.datetime64(pd.Timestamp(t)) for t in times], dtype="datetime64[ms]")
                        else:
                            times_dt = times
                        ...
                        logger.warning(f"Early exit time check failed: {exc}")
...
def _process_chunk_duckdb(
    ...
    times  = np.array(times_raw,  dtype='datetime64[ms]')
...
def _estimate_total_rows(path: Path, data_start: int) -> int:
    try:
        ...
        return max(1, int((get_file_size(path) - data_start) / avg))
    except Exception:
        return 1_000_000

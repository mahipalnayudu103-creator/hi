...
from utils.csv_stream    import stream_ticks, CSV_CHUNK_ROWS
from services.renko_state   import build_streaming_engines
from services.parquet_cache import (
...
from services.job_manager   import RenkoJob, SENTINEL
...
async def run_build_pipeline(
    ...
    try:
        ...
        # ── Copy per-pip Parquet files to base-key flat cache for future reuse ─
        try:
        ...
        # Merge any partially-cached pips into final counts before set_done
...
def _estimate_rows(path: Path) -> int:
    ...
    try:
        from utils.csv_stream import _estimate_total_rows
        from utils.csv_reader import open_compressed_file

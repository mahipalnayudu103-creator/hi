...
def configure_thread_pools() -> Dict[str, int]:
    ...
    Values from .env take priority (already loaded into os.environ by app.py).
    Must be called BEFORE any polars / duckdb import.
    """
    cpu_count = os.cpu_count() or 4
    polars_threads = int(os.environ.get("POLARS_MAX_THREADS", str(cpu_count)))
    duckdb_threads = int(os.environ.get("DUCKDB_THREADS", str(cpu_count)))

    os.environ["POLARS_MAX_THREADS"] = str(polars_threads)
    os.environ["RAYON_NUM_THREADS"]  = str(polars_threads)   # Polars Rayon backend
    os.environ["OMP_NUM_THREADS"]    = str(polars_threads)   # OpenMP
    os.environ["DUCKDB_THREADS"]     = str(duckdb_threads)

    logger.info(f"Thread pools configured: Polars={polars_threads}, DuckDB={duckdb_threads} threads.")
    return {"cpu_count": cpu_count, "polars_threads": polars_threads, "duckdb_threads": duckdb_threads}

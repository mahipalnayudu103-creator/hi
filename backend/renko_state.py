...
class RenkoState:
    ...
    def process_tick(
        ...
        while True:
            ...
            if price >= up_trigger - self._eps:
                # ── Upward brick ─────────────────────────────────────────────
                if self.direction < 0:
                    brick_open  = self.last_close + (reversal - 1) * self.brick_size
                    brick_close = self.last_close + reversal * self.brick_size
                else:
                    brick_open  = self.last_close
                    brick_close = self.last_close + self.brick_size

                brick_high = brick_close
                brick_low  = min(self.forming_low, brick_open, brick_close)
            ...
            if price <= down_trigger + self._eps:
                ...
                brick_high = max(self.forming_high, brick_open, brick_close)
    ...
    def process_ticks_batch(
        ...
        for idx in range(brick_count):
            ...
            confirmed.append({
                ...
                "confirm_time":       str(times[tick_idx - global_tick_start]),
    ...
    def get_live_brick(
        ...
        return {
            ...
            "high":              max(fh, current_price),
            "low":               min(fl, current_price),
            "close":             current_price,
            "direction":         "up" if current_price >= self.forming_open else "down",
    ...
    for i in range(n):
        ...
        while True:
            ...
            if price >= up_trigger - eps:
                # Upward brick
                if direction < 0:
                    brick_open = last_close + (reversal - 1) * brick_size
                    brick_close = last_close + reversal * brick_size
                else:
                    brick_open = last_close
                    brick_close = last_close + brick_size
                    
                brick_high = brick_close
                brick_low = min(forming_low, min(brick_open, brick_close))
            ...
            if price <= down_trigger + eps:
                ...
                brick_high = max(forming_high, max(brick_open, brick_close))
                brick_low = brick_close

...
DEFAULT_DELIMITER = ","
EXACT_ROW_COUNT_MAX_BYTES = 512 * 1024 * 1024
...
def estimate_csv_rows_from_head(path: Path, sample_rows: int = 10000) -> int:
    ...
    return max(0, int((path.stat().st_size - len(header)) / average_row_bytes))
...
def parse_csv_time_value(value: Any, base_year: int = 2026) -> pd.Timestamp | None:
    ...
    if raw_value.count(':') == 1 and '-' not in raw_value and '/' not in raw_value and ' ' not in raw_value:
        ...
        try:
            ...
            return base + pd.Timedelta(minutes=mins, seconds=secs)
        except Exception:
            pass
    ...
    if pd.api.types.is_string_dtype(s):
        ...
        if not non_nulls.empty:
            ...
            if sample_val.isdigit():
                ...
                return normalize_pandas_time_col(df, time_col, state, base_year)
            elif sample_val.count(':') == 1 and '-' not in sample_val and '/' not in sample_val and ' ' not in sample_val:
                # MM:SS format!
...
def summarize_csv_file(path: Path) -> dict[str, Any]:
    ...
    try:
        preview = pd.read_csv(path, nrows=25, sep=delimiter)
        if preview.empty:
            summary["status"] = "empty"
            return summary
...
def read_selected_range_duckdb(
    ...
    """
    DuckDB SQL-style CSV reader — second fallback after Polars.

    DuckDB parallelizes the CSV scan natively, uses predicate pushdown on
    timestamps, and handles edge-case CSV formats (mixed line-endings,
    BOM, quoted fields) that Polars sometimes rejects.

    Returns: (prices_np, times_np, rows_scanned, rows_loaded, engine_name)
    """
    import duckdb
    import numpy as np
    ...
    sep_char  = delimiter.replace("'", "''")   # escape for SQL
    ...
    # Read CSV via DuckDB — parallel, predicate-pushed
    con = duckdb.connect(database=":memory:")
    try:
        # DuckDB reads the CSV and filters time range in SQL
        sql = f"""
            SELECT
    ...
    # Truncate microseconds to milliseconds (23 chars)
    times_np = np.array(
    ...
    try:
        ...
        with path.open("rb") as handle:
            ...
            while True:
                ...
                if df_filtered.is_empty():
                    # If it's wrapping, we cannot assume subsequent chunks don't wrap back or match the range, so we must continue scanning the whole file!
                    if is_wrapping:
                        continue
                ...
                t_arr = df_filtered[time_col].dt.strftime("%Y-%m-%d %H:%M:%S.%f").to_numpy()
        ...
        with path.open("rb") as handle:
            ...
            while True:
                ...
                if not is_wrapping and parsed_times.min() >= end_t:
                    break
                if not is_wrapping and parsed_times.max() < start_t:
                    continue
                    
                mask = (parsed_times >= start_t) & (parsed_times < end_t)
                if not mask.any():
                    continue

...
class RenkoState:
        ...
        while True:
            ...
            if price >= up_trigger - eps:
                ...
                row_data = self._append_brick(
                    ...
                    wick_low=min(self.live_low, brick_open, brick_close),
                    time_str=time_str,
                    fill=UP_FILL,
                    line=UP_LINE,
                    direction="up",
            ...
            if price <= down_trigger + eps:
                ...
                row_data = self._append_brick(
                    brick_open=brick_open,
                    brick_close=brick_close,
                    wick_high=max(self.live_high, brick_open, brick_close),
    ...
    def get_live_brick(self, current_price: float, current_bid: float = 0.0, current_ask: float = 0.0) -> dict | None:
        ...
        return {
            ...
            "confirm_time": "",
            "open": self.live_open,
            "high": max(self.live_high, current_price) if self.live_high is not None else current_price,
            "low": min(self.live_low, current_price) if self.live_low is not None else current_price,
            "close": current_price,
            "direction": "up" if current_price >= self.live_open else "down",
            "tick_count": self.live_tick_count,
            "bid": current_bid,
            "ask": current_ask,
            "is_live": True
